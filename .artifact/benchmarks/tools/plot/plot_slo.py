        ...
        for system in systems:
            ...
            for slo in slo_requirements:
                slo_data.append({
                    ...
                    "slo": slo,
                    "percentage": len(sys_latency_df[sys_latency_df.time <= slo]) / len(sys_latency_df)
                })
        slo_df = pd.DataFrame(slo_data)
        fig2 = px.line(slo_df, x="slo", y="percentage", color="system", color_discrete_map=system_color_mapping)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--type", type=str, choices=['nfs', 'nvme'], help="Type of latency")
