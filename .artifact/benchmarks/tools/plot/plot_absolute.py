...
def plot(args):
    distributions = [x for x in os.listdir(args.input) if os.path.isdir(os.path.join(args.input, x))]
    for dist in distributions:
        arrival_rates = [x for x in os.listdir(os.path.join(args.input, dist)) if os.path.isdir(os.path.join(args.input, dist, x))]
        for ar in arrival_rates:
            ar_str = ar.split("=")[1]
            files = [x for x in os.listdir(os.path.join(args.input, dist, ar)) if x.endswith(".jsonl")]
            df = pd.DataFrame()
            distribution = []
            for file in files:
                metadata, data = parse_data(os.path.join(
                    args.input,
                    dist,
                    ar, 
                    file)
