...
class VocabParallelEmbeddingWithDelta(BaseLayerWithDelta):
    ...
    def create_delta_weights(self, max_deltas: int, delta_config: DeltaConfig,
        ...
        self.delta_weights = torch.zeros(
            max_deltas,
            self.base_layer.num_embeddings_per_partition,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        apply_delta_embed(masked_input, self.delta_weights, indices,
                          output_parallel)
...
class ColumnParallelLinearWithDelta(BaseLayerWithDelta):
    ...
    def set_delta(
        ...
        if self.tp_size > 1:
            pass
        self.qweight_stacked[index, 0, :, :].copy_(qweight,
                                                   non_blocking=ASYNC_COPY)
        self.qzero_stacked[index, 0, :, :].copy_(qzeros,
                                                 non_blocking=ASYNC_COPY)
        self.scales_stacked[index, 0, :, :].copy_(scales,
                                                  non_blocking=ASYNC_COPY)
    ...
    def set_delta(
        ...
        if self.tp_size > 1:
            ...
            if qweight[0] is not None:
                ...
                qzeros[0] = qzeros[0][:,
                                      start_idx // self.pack_factor:end_idx //
                                      self.pack_factor]
        ...
        if qweight[0] is not None:
            self.qweight_stacked[0][
                index, 0, :qweight[0].shape[0], :qweight[0].shape[1]].copy_(
                    qweight[0], non_blocking=ASYNC_COPY)
            self.qzeros_stacked[0][
                index, 0, :qzeros[0].shape[0], :qzeros[0].shape[1]].copy_(
                    qzeros[0], non_blocking=ASYNC_COPY)
            self.scales_stacked[0][
                index, 0, :scales[0].shape[0], :scales[0].shape[1]].copy_(
                    scales[0], non_blocking=ASYNC_COPY)
            self.g_idx[0] = g_idx[0]

        if qweight[1] is not None:
            self.qweight_stacked[1][
                index, 0, :qweight[1].shape[0], :qweight[1].shape[1]].copy_(
                    qweight[1], non_blocking=ASYNC_COPY)
            self.qzeros_stacked[1][
                index, 0, :qzeros[1].shape[0], :qzeros[1].shape[1]].copy_(
                    qzeros[1], non_blocking=ASYNC_COPY)
            self.scales_stacked[1][
                index, 0, :scales[1].shape[0], :scales[1].shape[1]].copy_(
                    scales[1], non_blocking=ASYNC_COPY)
    ...
    def set_delta(
        ...
        if self.tp_size > 1:
            if qweight[0] is not None:
                ...
                qzeros_q = qzeros[0][
                    :,
                    self.q_proj_shard_size * self.q_shard_id //
                    self.pack_factor:self.q_proj_shard_size *
                    (self.q_shard_id + 1) // self.pack_factor,
                ]
                scales_q = scales[0][
                    :,
                    self.q_proj_shard_size *
                ...
                self.qweight_stacked[0][
                    index, 0, :qweight_q.shape[0], :qweight_q.shape[1]].copy_(
                        qweight_q, non_blocking=ASYNC_COPY)
                self.qzeros_stacked[0][
                    index, 0, :qzeros_q.shape[0], :qzeros_q.shape[1]].copy_(
                        qzeros_q, non_blocking=ASYNC_COPY)
                self.scales_stacked[0][
                    index, 0, :scales_q.shape[0], :scales_q.shape[1]].copy_(
            ...
            if qweight[1] is not None:
                ...
                qzeros_q = qzeros[1][
                    :,
                    self.kv_proj_shard_size // self.pack_factor *
                    self.kv_shard_id:self.kv_proj_shard_size *
                    (self.kv_shard_id + 1) // self.pack_factor,
                ]
                scales_q = scales[1][
                    :,
                    self.kv_proj_shard_size *
                ...
                self.qweight_stacked[1][
                    index,
                    0, :qweight_kv.shape[0], :qweight_kv.shape[1]].copy_(
                        qweight_kv, non_blocking=ASYNC_COPY)
                self.qzeros_stacked[1][
                    index, 0, :qzeros_q.shape[0], :qzeros_q.shape[1]].copy_(
                        qzeros_q, non_blocking=ASYNC_COPY)
                self.scales_stacked[1][
            ...
            if qweight[0] is not None:
                self.qweight_stacked[0][
                    index,
                    0, :qweight[0].shape[0], :qweight[0].shape[1]].copy_(
                        qweight[0], non_blocking=ASYNC_COPY)
                # logger.info(f"Copying {qweight[0].nbytes/1024} kb from {qweight[0].device} to {self.qweight_stacked[0].device}")
            if qweight[1] is not None:
                self.qweight_stacked[1][
                    index,
                    0, :qweight[1].shape[0], :qweight[1].shape[1]].copy_(
                        qweight[1], non_blocking=ASYNC_COPY)

            if qweight[2] is not None:
                self.qweight_stacked[2][
                    index,
                    0, :qweight[2].shape[0], :qweight[2].shape[1]].copy_(
            ...
            if qzeros[2] is not None:
                self.qzeros_stacked[2][
                    index, 0, :qzeros[2].shape[0], :qzeros[2].shape[1]].copy_(
                        qzeros[2], non_blocking=ASYNC_COPY)
            ...
            if scales[2] is not None:
                self.scales_stacked[2][
                    index, 0, :scales[2].shape[0], :scales[2].shape[1]].copy_(
                        scales[2], non_blocking=ASYNC_COPY)
...
class RowParallelLinearWithDelta(BaseLayerWithDelta):
    ...
    def set_delta(
        ...
        if self.base_layer.tp_size > 1:
            ...
            qweight = qweight[start_idx // self.pack_factor:end_idx //
                              self.pack_factor, :]
            qzeros = qzeros[start_idx // self.pack_factor:end_idx //
                            self.pack_factor, :]
            scales = scales[start_idx:end_idx, :]

        self.qweight_stacked[index,
                             0, :qweight.shape[0], :qweight.shape[1]].copy_(
                                 qweight, non_blocking=ASYNC_COPY)
        self.qzeros_stacked[index,
                            0, :qzeros.shape[0], :qzeros.shape[1]].copy_(
                                qzeros, non_blocking=ASYNC_COPY)
        self.scales_stacked[index,
                            0, :scales.shape[0], :scales.shape[1]].copy_(
...
class LogitsProcessorWithDelta(BaseLayerWithDelta):
    ...
    def _get_logits(
        ...
        print(
            f"Hidden states: {hidden_states.shape}, embedding.shape: {embedding.shape}"
        )
        ...
        if logits is None:
            return None
        apply_delta_uncompressed(hidden_states, self.weight_stacked,
                                 self.indices[:self.indices_len[1]], logits)
...
_all_delta_classes: Set[Type[BaseLayerWithDelta]] = {
    ...
    and issubclass(cls, BaseLayerWithDelta) and cls is not BaseLayerWithDelta
}
...
def from_layer(
    ...
    for delta_cls in _all_delta_classes:
        if delta_cls.can_replace_layer(layer, delta_config,
...
def from_layer_logits_processor(
    ...
    ret = LogitsProcessorWithDelta(layer, lm_head.embedding_dim,
