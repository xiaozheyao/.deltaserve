...
def test_correctly_calls_draft_model(k: int, batch_size: int):
    ...
    worker = SpecDecodeWorker(
        draft_worker, target_worker, rejection_sampler, metrics_collector
    )
    ...
    for args, _ in call_args_list:
        ...
        actual_execute_model_data = ExecuteModelData(
            seq_group_metadata_list,
            blocks_to_swap_in,
            blocks_to_swap_out,
            blocks_to_copy,
...
def test_correctly_calls_target_model(k: int, batch_size: int):
    ...
    execute_model_data, prompts, prev_output_tokens = create_batch(batch_size, k)
    ...
    for args, kwargs in call_args_list:
        ...
        assert (
            len(target_execute_model_data.seq_group_metadata_list)
            == (k + 1) * batch_size
        )
        for seq_group_metadata in target_execute_model_data.seq_group_metadata_list:
...
def test_correctly_calls_rejection_sampler(k: int, batch_size: int):
    ...
    target_output = create_sampler_output_list(target_token_ids, target_token_probs)
    ...
    assert torch.equal(
        actual_bonus_token_ids, target_token_ids.reshape(batch_size, k + 1)[:, -1:]
    )
    assert torch.equal(
        actual_proposal_scores,
...
def test_correctly_formats_output(k: int, batch_size: int):
    ...
    rejection_sampler_output = torch.randint(
        low=0,
        high=vocab_size,
        size=(batch_size, k + 1),
        dtype=torch.int64,
...
def test_k_equals_zero(k: int, batch_size: int):
    ...
    assert out[0].sampled_tokens is None, "expect gpu tensor references to be None"

    draft_worker.execute_model.assert_called_once_with(
        **execute_model_data.to_dict(), return_python_output=False
    )
    target_worker.execute_model.assert_called_once_with(**execute_model_data.to_dict())
...
@torch.inference_mode()
def test_profile_num_available_blocks(
